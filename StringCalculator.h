...
// Skip over the newline if present
...
// Get the starting position of the actual number string in the input
const char* get_start_position(const char* input) {
    return skip_double_slash(skip_newline(input));
}

// Replace newlines with commas in the input string for uniform delimiter handling
...
// Extract the number portion from the input string
...
// Parse the input string to extract the delimiter and numbers
...
// Split the number string based on the delimiter and convert to integers
...
// Check if the array of numbers contains any negative values
...
// Construct an error message for negative numbers
...
// Check for negative numbers and print an error message if found
...
// Sum valid numbers (ignoring those greater than 1000)
